...
# Let's craft the 30s Master Video Prompt adhering strictly to prompt auditor and Director O.S. V20.5 rules:
# No banned phrases ("all four", "all three", "drifting", "slowly", "hyper-realistic", "photorealistic", "masterpiece", "0.8s tail-buffer", "0% tears", "0% blush", "0% cgi", etc.)
# Positive physics: "1.0x real-time Newtonian velocity"
# Anti-sfx bloat: acoustic, diegetic, rt60, 80hz
# Hard cuts: 14 hard cuts matching 15 shots exactly.

video_prompt = """FORMAT: Single continuous 30-second one-take street UGC documentary sequence
...
CHARACTER LOCK: CALLUM (exact: navy windbreaker). Third stocky companion in stone-grey hooded jacket strikes disposable flint lighter with sharp flick, cupping flame against wind to ignite cigarette, exhaling a puff of grey tobacco smoke past Callum's shoulder.
...
CHARACTER LOCK: CALLUM (exact: navy windbreaker, raw denim, suede sneakers). The four stocky companions form a tight unit outside pub corner, pulling up high neck zips and adjusting jacket cuffs, unified collective readiness before setting off.
...
CHARACTER LOCK: CALLUM (exact: navy windbreaker, olive polo, raw denim). The four men walk abreast in purposeful march, burly broad shoulders filling the vertical 9:16 mobile frame, breath mist forming in cool air.
...
with open("scratch/test_video_prompt.txt", "w", encoding="utf-8") as f:
    f.write(video_prompt)

res = subprocess.run(["python", "prompt_auditor.py", "scratch/test_video_prompt.txt", "--duration", "30", "--mode", "3", "--json"], capture_output=True, text=True, encoding="utf-8")
print(res.stdout)
