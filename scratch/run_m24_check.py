...
prompt_m24 = """TAGS: @Image1 (Subject Identity & Wardrobe - Callum 4-Panel Character Sheet) | @Image2 (Environment Set Architecture & Lighting - Stadium Approach Alleyway)

FORMAT: Single continuous 30-second one-take street UGC documentary sequence
...
LOCATION — ENV_ID [STREET_STADIUM_APPROACH]: Extract architectural set and atmosphere from @Image2. Damp Victorian red-brick industrial alleyway leading to concrete stadium concourse plaza, 16:45 overcast late afternoon, cool 5200K diffused daylight. Wet asphalt pavement with puddle reflections and cast-iron manhole covers. Left boundary: red Victorian brick facade and weathered pub doorway. Right boundary: rusted corrugated steel perimeter and lampposts. Background depth elements: concrete stadium grandstands and illuminated 60-meter floodlight pylons piercing the grey sky. Strict Crowd Quota: Exactly 6 distant background supporters walking toward turnstiles (0% static crowd, 0% morphing).

GLOBAL CHARACTER LOCK — CALLUM: Extract facial identity, hair geometry, and complete wardrobe from @Image1. 29yo British terrace casual supporter, burly stocky robust build, 182cm, broad shoulders, weathered masculine jawline, short textured taper crop dark hair, subtle 2-day beard stubble, observant slate-blue eyes with specular catchlights, natural skin micro-pores and cold-air cheek flushing.
...
PROPS: Stainless steel sports chronograph watch on left wrist; silver signet ring on right pinky finger; box of matches in left pocket.
...
FRAMING DIRECTIVE: Handheld mobile framing at chest and eye level; 180-degree spatial axis preserved.
...
CHARACTER LOCK: CALLUM @Image1 (exact: navy windbreaker, raw denim). Callum steps forward from wall, clasping hands firmly in traditional terrace grip with incoming stocky friend in olive Harrington jacket, exchanging respectful head nods.
...
CHARACTER LOCK: CALLUM @Image1 (exact: navy windbreaker). Third stocky companion in stone-grey hooded jacket strikes disposable flint lighter with sharp flick, cupping flame against wind to ignite cigarette, exhaling a puff of grey tobacco smoke past Callum's shoulder.
...
CHARACTER LOCK: CALLUM @Image1 (exact: navy windbreaker, raw denim, suede sneakers). The four stocky companions form a tight unit outside pub corner, pulling up high neck zips and adjusting jacket cuffs, unified collective readiness before setting off.
...
CHARACTER LOCK: CALLUM @Image1 (exact: navy windbreaker, raw denim). Group strides with synchronized heavy cadence; Callum occupies center-left, hands thrust into jacket slash pockets, eyes locked forward toward distant stadium floodlights @Image2.
...
CHARACTER LOCK: CALLUM @Image1 (exact: navy technical windbreaker). Handheld camera tracks beside Callum's chest, capturing tactile micro-ripstop crinkle nylon fabric, heavy molded zipper teeth, and woven utility chest pocket seam details as fabric shifts with his gait.
...
CHARACTER LOCK: CALLUM @Image1 (exact: suede gum-sole sneakers, raw denim cuff). Callum's navy suede gum-sole trainers step firmly across asphalt edge and splash through shallow puddle water, followed closely by companions' leather terrace boots.
...
CHARACTER LOCK: CALLUM @Image1 (exact: navy windbreaker, olive polo, raw denim). The four men walk abreast in purposeful march, burly broad shoulders filling the vertical 9:16 mobile frame, breath mist forming in cool air.
...
CHARACTER LOCK: CALLUM @Image1 (exact: navy windbreaker). Callum raises his right hand, gripped with silver signet ring, grasping metal zipper puller and fastening storm collar all the way up to chin level, fabric stretching taut across muscular neck.
...
CHARACTER LOCK: CALLUM @Image1 (exact: navy windbreaker back yoke, raw denim). Camera captures ergonomic storm-flap back construction of navy jacket, structured taper cut of raw denim, and relaxed rolling gait of a seasoned matchday regular.
...
CHARACTER LOCK: CALLUM @Image1 (exact: navy windbreaker, raw denim). Massive concrete stadium grandstand and luminous 60-meter floodlight towers emerge into view @Image2, casting soft high-angle spill across wet tarmac.
...
CHARACTER LOCK: CALLUM @Image1 (exact: navy windbreaker unzipped two inches showing olive polo collar). Side angle highlights crisp ribbed collar of olive cotton polo snug beneath windbreaker neck, jawline firm and determined.
...
CHARACTER LOCK: CALLUM @Image1 (exact: navy windbreaker, raw denim, suede sneakers). Callum guides his path past steel barricade, companions falling into single-file entry formation outside turnstile plaza @Image2.
...
CHARACTER LOCK: CALLUM @Image1 (exact: navy windbreaker, signet ring on right hand). Callum extends right hand to push revolving steel bar of mechanical stadium turnstile, stepping into narrow concrete concourse portal.
...
with open("scratch/test_m24.txt", "w", encoding="utf-8") as f:
    f.write(prompt_m24)

res = subprocess.run(["python", "prompt_auditor.py", "scratch/test_m24.txt", "--duration", "30", "--mode", "3", "--json"], capture_output=True, text=True, encoding="utf-8")
print(res.stdout)
